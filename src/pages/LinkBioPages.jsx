...
export default function LinkBioPages() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => {
      if (editingPage) {
        return base44.entities.LinkBioPage.update(editingPage.id, data);
      }
      return base44.entities.LinkBioPage.create(data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['linkbio-pages'] });
      handleCloseDialog();
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.LinkBioPage.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['linkbio-pages'] });
  ...
  const handleEdit = (page) => {
    ...
    setFormData({
      ...
      background_color: page.background_color || "#6366f1",
      status: page.status,
    });
    setSelectedPetitions(page.petition_ids || []);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/20 to-purple-50/20">
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-8">
                  ...
                  <div className="relative">
                    ...
                    <DialogTitle className="text-3xl font-black text-white mb-2">
                      {editingPage ? 'Editar Página LinkBio' : 'Criar Página LinkBio'}
                    </DialogTitle>
                    <DialogDescription className="text-white/90 text-base">
                      {editingPage ? 'Atualize as informações da sua página.' : 'Agrupe suas petições em uma página personalizada e compartilhe com seu público.'}
                    </DialogDescription>
                  </div>
                </div>
                      ...
                      <div className="flex gap-3 pt-4">
                        ...
                        >
                          {createMutation.isPending ? (
                            <>
                              <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                              {editingPage ? 'Atualizando...' : 'Publicando...'}
                            </>
                          ) : (
                            <>
                              <Rocket className="w-4 h-4 mr-2" />
                              {editingPage ? 'Atualizar' : 'Publicar'}
                            </>
                          )}
                        </Button>
                      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 -mt-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {currentPages.map((page) => {
            const pageUrl = `${window.location.origin}${createPageUrl(`bio?s=${page.slug}`)}`;
            ...
            return (
              <Card key={page.id} className="border-none shadow-lg hover:shadow-xl transition-all bg-white overflow-hidden">
                <div
                ...
                <CardHeader className="-mt-12 relative z-10">
                  <div className="w-20 h-20 rounded-full bg-white shadow-2xl border-4 border-white flex items-center justify-center mb-3 overflow-hidden">
                    {page.avatar_url ? (
                      <img src={page.avatar_url} alt="Page Avatar" className="w-full h-full object-cover" />
                    ) : (
                      <Link2 className="w-10 h-10" style={{ color: page.background_color }} />
                    )}
                  </div>
          ...
          <Card className="border-none shadow-lg bg-white mb-8">
            <CardContent className="p-16 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Crie sua primeira página LinkBio para agrupar suas petições
              </p>
              <Button
      ...
      <ConfirmDialog
        ...
        title="Excluir Página LinkBio"
        description={`Tem certeza que deseja excluir a página "${pageToDelete?.title}"? Esta ação não pode ser desfeita.`}
        confirmText="Excluir"
        variant="destructive"
      />
    </div>
