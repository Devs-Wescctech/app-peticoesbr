...
export default function PetitionLanding() {
  ...
  const slug = urlParams.get('s'); // Mudou de 'id' para 's' (slug)
  ...
  const signMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowSuccessModal(true); // Changed to showSuccessModal
  ...
  const primaryColor = petition.primary_color || "#6366f1"; // Added primaryColor
  const landingUrl = window.location.href; // Added landingUrl
  
  const shareText = petition.share_text 
    ? petition.share_text.replace('{link}', landingUrl)
    : `Acabei de assinar "${petition.title}". Junte-se a mim! ${landingUrl}`; // Added shareText
      ...
      <div className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-20">
          <div className="max-w-5xl mx-auto">
            ...
            {/* Sign Form - Modernizado com Backdrop Blur */}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {socialShares.map((social) => (
                      <button
                        ...
                        onClick={() => {
                          handleShare(social);
                          // Optionally close modal after share, but keep it open for more shares
